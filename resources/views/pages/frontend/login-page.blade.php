...
@section('subhead')
    <title>Front End Login Page - PC Care</title>
@endsection

@section('subcontent')
    @foreach ($loginPage as $item)
        <form action="{{ url('update-login-page/' . $item->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            {{-- if make update data, use source bellow --}}
            <input type="hidden" name="_method" value="PUT">
            <div class="intro-y flex items-center mt-8">
                <h2 class="text-lg font-medium mr-auto">Login Page</h2>
